...
class TestClearExpenses:
    ...
    def test_delete_expense(self, temp_file):
        ...
        append_expense(
            "Food", 10.00, "Lunch", temp_file, timestamp="2026-01-03T12:00:00+00:00"
        )
        append_expense(
            "Rent", 500.00, "Monthly", temp_file, timestamp="2026-01-03T12:00:01+00:00"
        )

        deleted = delete_expense(
            "Food", 10.00, "Lunch", "2026-01-03T12:00:00+00:00", path=temp_file
        )
        assert deleted
        expenses = load_expenses(temp_file)
        assert len(expenses) == 1
